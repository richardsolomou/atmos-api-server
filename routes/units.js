module.exports = function (server, mysql_conn, prefix, restify) {
	...
	server.get(prefix + '/units/:unit_id/students', function (req, res, next) {
		mysql_conn.query('SELECT * FROM `units` WHERE `unit_id` = :unit_id', { unit_id: req.params.unit_id }, function (err, results) {
			if (err) return next(err);
			if (!results || !results.length) return next(new restify.errors.NotFoundError('Invalid unit ID.'));

			mysql_conn.query('SELECT `students`.* FROM `studentsessions` INNER JOIN `sessions` ON `studentsessions`.`session_id` = `sessions`.`session_id` INNER JOIN `students` ON `studentsessions`.`student_id` = `students`.`student_id` WHERE `sessions`.`unit_id` = :unit_id GROUP BY `students`.`student_id`', { unit_id: req.params.unit_id }, function (err, results) {
				if (err) return next(err);
	...
	server.get(prefix + '/units/:unit_id/sessions', function (req, res, next) {
		mysql_conn.query('SELECT * FROM `units` WHERE `unit_id` = :unit_id', { unit_id: req.params.unit_id }, function (err, results) {
			...
			mysql_conn.query('SELECT * FROM `sessions` WHERE `unit_id` = :unit_id', { unit_id: req.params.unit_id }, function (err, results) {
				if (err) return next(err);
	...
	server.get(prefix + '/units', function (req, res, next) {
		mysql_conn.query('SELECT * FROM `units`', function (err, results) {
			if (err) return next(err);
